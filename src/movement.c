...
/*   Updated: 2024/04/17 19:46:19 by lade-kon      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	check_if_collectable_or_exit(t_game *game, char **map, int x, int y)
{
	...
	if (map[y][x] == 'C')
	{
		...
		ft_printf(B_G"Good job!! "B"You collected a"Y" flower.\n"DEF);
		ft_printf("Collected: %i\tTo collect: %i\n", game->collected, remain);
		i = 0;
		while (i < game->collectables)
		{
			if (x == (game->img.flower->instances[i].x / PIXEL)
				&& y == (game->img.flower->instances[i].y / PIXEL))
			{
				game->img.flower->instances[i].enabled = false;
				break ;
			}
			i++;
		}
